...
def openbookview():
    def show():
        bookno = inputtxt2.get("1.0", "end-1c")
        c.execute("SELECT EXISTS(SELECT 1 FROM books WHERE book_name=? LIMIT 1)", (bookno,))
        check = c.fetchone()
        if check[0] == 1:
            print("Name is in the table")
            c.execute("SELECT * FROM books WHERE book_name=?",(bookno,))
            entry = c.fetchone()
